...
import { listMembers, inviteMember, updateMemberRole, removeMember } from "./membersApi";
...
useEffect(refresh, [activeOrgId]);
  ...
  async function handleInvite() {
    ...
    setInviteLink(`${window.location.origin}/invitations/${invite.token}/accept`);
    setInviteEmail("");
  }
    ...
    <div className="p-8 max-w-3xl">
      ...
      {inviteLink && (
        <div className="bg-accent/10 border border-accent/30 rounded-md p-3 mb-6 text-xs">
          <p className="text-muted mb-1">Share this invite link (no email service configured yet):</p>
      ...
      <div className="space-y-2">
        {members.map((m) => (
          <div key={m.userId} className="flex items-center gap-3 bg-surface border border-border rounded-md px-4 py-3">
            ...
            {canManage && m.role !== "OWNER" && (
              <button onClick={() => handleRemove(m.userId)} className="text-xs text-muted hover:text-danger">
                Remove
              </button>
            )}
          </div>
